...
// DamagePack represents a purchasable damage upgrade that appears in the world. It has a tier (bronze, silver, gold) that determines its cost and buff multiplier. The player can purchase it by colliding with it and having enough coins.
public class DamagePack {
    ...
    public DamagePack(int worldX, int worldY, Tier tier) {
        ...
        switch (tier) {
            ...
            case SILVER -> {
                cost = 5;
                buffMultiplier = 1.50f;
            }
            default -> {
                cost = 8;
                buffMultiplier = 2.00f;
            }
        }
    }
    ...
    private Color tierBg() {
        return switch (tier) {
            case BRONZE -> C_BRONZE_BG;
            case SILVER -> C_SILVER_BG;
            default -> C_GOLD_BG;
        };
    }

    private Color tierFg() {
        return switch (tier) {
            case BRONZE -> C_BRONZE_FG;
            case SILVER -> C_SILVER_FG;
            default -> C_GOLD_FG;
        };
    }

    private Color tierRim() {
        return switch (tier) {
            case BRONZE -> C_BRONZE_RIM;
            case SILVER -> C_SILVER_RIM;
            default -> C_GOLD_RIM;
        };
    }
}